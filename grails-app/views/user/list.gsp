			...
			<table>
				<thead>
					<tr>
						...
						<g:sortableColumn property="passwdHash" title="${message(code: 'user.passwdHash.label', default: 'Passwd Hash')}" />
				...
				<g:each in="${userInstanceList}" status="i" var="userInstance">
					<tr class="${(i % 2) == 0 ? 'even' : 'odd'}">
						...
						<td><g:link action="show" id="${userInstance.id}">${fieldValue(bean: userInstance, field: "secondName")}</g:link></td>
					
						<td>${fieldValue(bean: userInstance, field: "phone")}</td>
					
						<td>${fieldValue(bean: userInstance, field: "passwdHash")}</td>
